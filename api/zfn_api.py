...
class Client:
    ...
    def encrypt_password(cls, pwd, n, e):
        ...
        # [本地修改] 上游原代码为 `result = binascii.b2a_base64(encropy_pwd)`，
        # 但 binascii.b2a_base64() 的返回值末尾自带一个 "\n"，该换行会被一并
        # 当作 mm 参数提交给教务系统，部分学校的服务端会因此判定密码错误
        # （表现为 login 返回 code=1002「用户名或密码不正确」）。
        # 这里 strip() 掉尾部换行后再返回，不影响其它兼容该换行的学校。
        result = binascii.b2a_base64(encropy_pwd).strip()
        return result
